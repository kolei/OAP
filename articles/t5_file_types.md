...
>Как выяснилось на практике, это пространство имен не включено по-умолчанию в .NET. Его можно импортировать с помощью ***NuGet***, но на *WorldSkils* и демо-экзамене интернет недоступен, поэтому ниже мы рассмотрим другой вариант работы с JSON. 
...
var PersonList = new List<Person>() {
    new Person("Иванов", 25),
    new Person("Петров", 35)
};

var Serializer = new DataContractJsonSerializer(typeof(Person[]));

using (var stream = new StreamWriter(@"./test.json"))
{
    Serializer.WriteObject(stream.BaseStream, PersonList.ToArray());
}
```

А ещё можно сериализовать объект в XML:

```cs
using (var writer = new XmlTextWriter("test.xml", null))
{
    Serializer.WriteObject(writer, PersonList.ToArray());
}
```

**Десериализация** (Преобразование JSON-строки в объект)

Десериализация делается методом *ReadObject*, который на входе принимает поток с JSON-строкой (мне лень было цеплять файл, поэтому я показываю как разобрать обычную строку)
